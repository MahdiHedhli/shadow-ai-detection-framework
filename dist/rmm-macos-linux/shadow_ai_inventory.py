...
def collect_chromium_extension_profile(
    ...
    state_extension_ids: set[str] = set()
    for state_directory_name in ("Local Extension Settings", "Sync Extension Settings"):
        state_root = profile / state_directory_name
        if not safe_exists(state_root) or not state_root.is_dir():
            continue
        for extension_id in known_extensions:
            state_path = state_root / extension_id
            if safe_exists(state_path) and state_path.is_dir():
                state_extension_ids.add(extension_id)
    indexed_extensions = preference_extensions(profile)
    extension_ids = set(extension_paths) | set(indexed_extensions) | state_extension_ids
    ...
    for extension_id in sorted(extension_ids):
        ...
        if catalog_item and catalog_item["browser"] in {browser, "chromium-family"}:
            ...
            elif extension_id in indexed_extensions:
                classification_basis = "browser_preferences_catalog_id"
            else:
                classification_basis = "browser_extension_state_catalog_id"
        else:
