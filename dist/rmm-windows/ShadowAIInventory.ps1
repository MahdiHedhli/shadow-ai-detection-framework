...
function Collect-InstalledSoftware {
    ...
    try {
        foreach ($software in @(Get-ItemProperty -Path $registryPaths -ErrorAction SilentlyContinue | Where-Object { $_.DisplayName })) {
            foreach ($mapping in $nameIndicators) {
                if (([string]$software.DisplayName).IndexOf($mapping.Pattern, [StringComparison]::OrdinalIgnoreCase) -ge 0) {
                    Add-Finding -Category 'software' -Indicator $mapping.Indicator -SubjectUser $null -Attributes @{
                        display_name = [string]$software.DisplayName
                        version = [string]$software.DisplayVersion
